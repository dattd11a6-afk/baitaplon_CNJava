    ...
    <main class="flex-grow-1 d-flex flex-column" style="height: 100vh; overflow-y: auto;">
        ...
        <div class="p-4 px-5">
            <div class="d-flex justify-content-between align-items-end mb-4">
                <div><h2 class="fw-bold m-0 brand-font text-dark">Hội Viên & Khách Hàng</h2></div>
            </div>

            <div class="admin-card p-0">
                <table class="admin-table mb-0">
                    <thead>
                        <tr><th>Khách hàng</th><th>Liên hệ</th><th>Ngày tham gia</th><th class="col-number">Tổng chi tiêu</th><th class="text-center">Hạng thành viên</th><th>Trạng thái</th><th class="text-end">Thao tác</th></tr>
                    </thead>
                    <tbody>
                        <c:forEach var="c" items="${customers}">
                            <tr>
                                <td class="fw-bold text-dark">${c.fullName}</td>
                                <td><div class="text-dark">${c.phone}</div><div class="text-muted" style="font-size: 12px;">${c.email}</div></td>
                                ...
                                <td class="text-center"><span class="badge tier-${c.tier} px-3 py-1 rounded-pill" style="font-size: 11px; letter-spacing: 0.5px;">${c.tier}</span></td>
                                <td>
                                    <c:choose>
                                ...
                                <td class="text-end">
                                    <form action="${pageContext.request.contextPath}/admin/customers" method="POST" class="d-inline" onsubmit="return confirm('Bạn có chắc muốn ${c.status == 'ACTIVE' ? 'KHÓA' : 'MỞ KHÓA'} tài khoản này?');">
                                        <input type="hidden" name="action" value="toggleStatus"><input type="hidden" name="id" value="${c.id}"><input type="hidden" name="currentStatus" value="${c.status}">
                                        <c:choose>
                                            <c:when test="${c.status == 'ACTIVE'}"><button type="submit" class="btn btn-sm btn-light border text-danger" title="Khóa tài khoản"><i class="ph-bold ph-lock-key"></i> Khóa</button></c:when>
                                            <c:otherwise><button type="submit" class="btn btn-sm btn-light border text-success" title="Mở khóa"><i class="ph-bold ph-lock-key-open"></i> Mở khóa</button></c:otherwise>
                                        </c:choose>
                                    </form>
                                </td>
                            </tr>
