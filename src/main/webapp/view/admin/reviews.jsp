...
<head>
    <meta charset="UTF-8">
    <title>Quản lý Đánh giá | Fruit Admin</title>
    ...
    <style>
        ...
        /* ĐỒNG BỘ CSS SIDEBAR CHUẨN TỪ DASHBOARD */
        ...
        .admin-table th { padding: 16px 24px; color: #6B7280; background: #F9FAFB; text-transform: uppercase; font-size: 11px; font-weight: 600; border-bottom: 1px solid var(--border-color); text-align: left; }
...
<body>
<div class="d-flex">
    <!-- ĐỒNG BỘ HOÀN TOÀN KHỐI SIDEBAR TỪ DASHBOARD.JSP -->
    <aside class="sidebar">
        ...
        <div class="overflow-auto flex-grow-1 pb-4 custom-scrollbar">
            <ul class="sidebar-menu">
                ...
                <!-- BẬT SÁNG NÚT ĐÁNH GIÁ KHI Ở TRANG NÀY -->
                <li class="active"><a href="${pageContext.request.contextPath}/admin/reviews"><i class="ph-fill ph-star"></i> Đánh giá</a></li>
    ...
    <main class="flex-grow-1 overflow-auto" style="height: 100vh;">
        ...
        <div class="p-4 px-5">
            <h2 class="fw-bold mb-4 brand-font text-dark">Quản lý Đánh giá</h2>
                    ...
                    <tbody>
                        <c:forEach var="r" items="${reviews}">
                            <tr>
                                ...
                                <td><div class="text-truncate fw-medium text-success" style="max-width: 200px;" title="${r.productName}">${r.productName}</div></td>
                                <td class="text-warning" style="font-size: 18px;">
                                    <c:forEach begin="1" end="${r.rating}"><i class="ph-fill ph-star"></i></c:forEach>
                                </td>
                                <td>
                                    <div style="font-size: 14px;" class="mb-2 text-dark">${r.comment}</div>
                                    <c:if test="${not empty r.mediaUrl}">
                                        <a href="${pageContext.request.contextPath}/assets/images/reviews/${r.mediaUrl}" target="_blank" class="badge bg-light text-primary border border-primary-subtle text-decoration-none py-1 px-2"><i class="ph ph-image me-1"></i> Xem đính kèm</a>
                                    </c:if>
                                    <c:if test="${not empty r.sellerReply}">
                                        <div class="mt-3 p-3 bg-light border-start border-success border-3 text-muted rounded-end" style="font-size: 13px;">
                                            <div class="fw-bold text-success mb-1"><i class="ph-fill ph-storefront"></i> Cửa hàng đã phản hồi:</div>
                                            ${r.sellerReply}
                                        </div>
                                    </c:if>
                                </td>
                                <td class="text-end">
                                    <button class="btn btn-sm ${not empty r.sellerReply ? 'btn-light border text-primary' : 'btn-success'}" data-bs-toggle="modal" data-bs-target="#replyModal${r.id}">
                                        <i class="ph ph-chat-circle-dots"></i> ${not empty r.sellerReply ? 'Sửa phản hồi' : 'Phản hồi ngay'}
                                    </button>
                                </td>
                            </tr>
                                ...
                                <div class="modal-dialog modal-dialog-centered">
                                    <div class="modal-content border-0 rounded-4 shadow">
                                        <form action="${pageContext.request.contextPath}/admin/reviews" method="POST">
                                            <div class="modal-header border-bottom-0"><h5 class="fw-bold m-0 brand-font text-dark">Trả lời khách hàng</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
                                            <div class="modal-body text-start">
                                                <input type="hidden" name="reviewId" value="${r.id}">
                                                <div class="p-3 bg-light rounded-3 mb-3 border text-dark" style="font-size: 13px;">
                                                    <span class="text-muted d-block mb-1">Khách hàng nhận xét:</span>
                                                    " ${r.comment} "
                                                </div>
                                                <label class="form-label fw-medium text-dark">Nội dung phản hồi từ Cửa hàng</label>
                                                <textarea name="sellerReply" class="form-control" rows="4" placeholder="Cảm ơn bạn đã tin tưởng mua hàng..." required>${r.sellerReply}</textarea>
                                            </div>
                                            <div class="modal-footer border-0 pt-0"><button type="submit" class="btn btn-success w-100 fw-bold py-2">Gửi phản hồi</button></div>
                                        </form>
                                    </div>
                                </div>
                            </div>
...
<div class="toast-container position-fixed bottom-0 end-0 p-4" style="z-index: 1100;">
    <c:if test="${not empty sessionScope.successMsg}">
        <div class="toast align-items-center text-bg-success border-0 shadow" role="alert" aria-live="assertive" aria-atomic="true">
            <div class="d-flex"><div class="toast-body fw-medium px-3 py-2"><i class="ph-fill ph-check-circle me-2 fs-5"></i> ${sessionScope.successMsg}</div><button type="button" class="btn-close btn-close-white me-3 m-auto" data-bs-dismiss="toast"></button></div>
